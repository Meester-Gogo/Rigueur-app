...
const defaultState = () => ({
  ...
  dailyDefs: [
    { id: "d1", label: "10 min de rangement après le sport", xp: 8 },
    { id: "d2", label: "Zéro vaisselle qui traîne ce soir", xp: 8 },
    { id: "d3", label: "Un carton / une pile de travaux en moins", xp: 10 },
  ],
  customCounter: 0,
});
...
function isDayComplete(dailyDefs, dailyLog, dateStr) {
  const day = dailyLog[dateStr];
  if (!day || dailyDefs.length === 0) return false;
  return dailyDefs.every((d) => !!day[d.id]);
}

function computeStreak(dailyDefs, dailyLog, today) {
  let streak = 0;
  let cursor = today;
  if (isDayComplete(dailyDefs, dailyLog, cursor)) {
    streak = 1;
  }
  cursor = addDays(cursor, -1);
  while (isDayComplete(dailyDefs, dailyLog, cursor)) {
    streak += 1;
    cursor = addDays(cursor, -1);
  }
  return streak;
...
function normalizeAndRoll(loaded) {
  ...
  // migrate old shape (single "dailies" array with done flags + lastDailyDate)
  // into the new date-keyed dailyLog history, so old backups keep working.
  if (!Array.isArray(loaded.dailyDefs) || typeof loaded.dailyLog !== "object" || loaded.dailyLog === null) {
    const oldDailies = Array.isArray(loaded.dailies) ? loaded.dailies : fresh.dailyDefs.map((d) => ({ ...d, done: false }));
    loaded.dailyDefs = oldDailies.map(({ id, label, xp }) => ({ id, label, xp }));
    loaded.dailyLog = loaded.dailyLog && typeof loaded.dailyLog === "object" ? loaded.dailyLog : {};
    const anchorDate = loaded.lastDailyDate || todayStr();
    if (!loaded.dailyLog[anchorDate]) {
      loaded.dailyLog[anchorDate] = {};
      oldDailies.forEach((d) => {
        loaded.dailyLog[anchorDate][d.id] = !!d.done;
      });
    }
  }
  delete loaded.dailies;
  delete loaded.lastDailyDate;
  delete loaded.dailyAllDoneYesterday;

  // always land back on "today" when (re)opening; user can navigate to past days manually
  loaded.viewDate = todayStr();
  loaded.streak = computeStreak(loaded.dailyDefs, loaded.dailyLog, todayStr());
...
function RigueurRPG() {
  ...
  const toggleDaily = (id) => {
    update((next) => {
      const vd = next.viewDate || todayStr();
      if (!next.dailyLog[vd]) next.dailyLog[vd] = {};
      const wasDone = !!next.dailyLog[vd][id];
      next.dailyLog[vd][id] = !wasDone;
      const def = next.dailyDefs.find((d) => d.id === id);
      if (!wasDone) {
        gainXP(def.xp, next);
        showToast(vd === todayStr() ? `+${def.xp} XP` : `+${def.xp} XP (rattrapage ${vd})`);
      } else {
        next.xp = Math.max(0, next.xp - def.xp);
      }
      next.streak = computeStreak(next.dailyDefs, next.dailyLog, todayStr());
    });
  };

  const navigateDay = (delta) => {
    update((next) => {
      const cur = next.viewDate || todayStr();
      const target = addDays(cur, delta);
      if (target <= todayStr()) {
        next.viewDate = target;
      }
    });
  };
  ...
  const currentViewDate = state.viewDate || todayStr();
  const viewLog = state.dailyLog[currentViewDate] || {};
  const totalDailies = state.dailyDefs.length;
  const doneDailies = state.dailyDefs.filter((d) => viewLog[d.id]).length;
  ...
  return (
    <div className="rpg-root">
      <style>{`
        ...
        .day-nav button {
          background: var(--bg-panel-2); border: 1px solid rgba(217,165,68,0.25); color: var(--ink);
        ...
        .day-nav .label { color: var(--amber); font-weight: bold; min-width: 90px; text-align: center; }
        .toast {
      ...
      <div className="day-nav">
        ...
        <span className="label mono">{formatDayLabel(currentViewDate)}</span>
        <button onClick={() => navigateDay(1)} disabled={currentViewDate >= todayStr()}>›</button>
      </div>
      <div className="card">
        {state.dailyDefs.map((d) => {
          const done = !!viewLog[d.id];
          return (
            <div key={d.id} className={`quest-row ${done ? "done" : ""}`} onClick={() => toggleDaily(d.id)}>
              {done ? <CheckCircle2 size={18} color="#5B8C7B" /> : <Circle size={18} color="#93A29C" />}
      ...
      {currentViewDate !== todayStr() && (
        <div className="commit-help" style={{ marginTop: -6, marginBottom: 14 }}>
          Tu modifies le {formatDayLabel(currentViewDate)} — reviens à "Aujourd'hui" avec › pour continuer ta journée en cours.
        </div>
      )}
